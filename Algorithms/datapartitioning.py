...
def rangepartition(ratingstablename, numberofpartitions, openconnection):
    ...
    cur = openconnection.cursor()
    # creating range metadata
    try:
        query = "CREATE TABLE {0} ( {1} real, {2} real,{3} integer);".format(RANGE_METADATA,
                                                                             "minrating", "maxrating", "partitionnum")
        cur.execute(query)
    except Exception as ex:
        print("Failed to create table: ", ex)
    print("Created table:  ", RANGE_METADATA)
    ...
    partitioninterval = abs(
        ratingupperbound - ratinglowerbound) / numberofpartitions
    ...
    for i in range(0, numberofpartitions):
        ...
        metaDataQuery = " INSERT INTO {0} VALUES ({1}, {2}, {3})".format(
            RANGE_METADATA, lowerbound, upperbound, i)
...
def roundrobinpartition(ratingstablename, numberofpartitions, openconnection):
    ...
    cur = openconnection.cursor()
    try:
        query = "CREATE TABLE {0} ( {1} integer);".format(RROBIN_METADATA,"partitionnum")
        cur.execute(query)
    except Exception as ex:
        print("Failed to create table: ", ex)
    print("Created table:  ", RROBIN_METADATA)

    try:
        query = "INSERT INTO {0} VALUES ({1})".format(RROBIN_METADATA,numberofpartitions)
        cur.execute(query)
    except Exception as ex:
        print("Failed to insert into table: ", ex)
    print("Inserted into table:  ", RROBIN_METADATA)
...
def roundrobininsert(ratingstablename, userid, itemid, rating, openconnection):
    ...
    cur.execute(query)
    openconnection.commit()
    print("Inserted value in partition: ",
          RROBIN_TABLE_PREFIX + repr(partitiontoinsert))
    pass
...
def rangeinsert(ratingstablename, userid, itemid, rating, openconnection):
    ...
    ratingupperbound = 5.0
    partitioninterval = abs(
        ratingupperbound - ratinglowerbound) / partitioncount
    ...
    openconnection.commit()
    print("Inserted value in partition: ",
          RANGE_TABLE_PREFIX + repr(partitiontoinsert))
    pass
...
def tablecount(cur, tableprefix):
    query = "SELECT COUNT(table_name) FROM information_schema.tables WHERE table_schema = 'public' AND                                                                         table_name LIKE '{0}%';".format(
        tableprefix)
    cur.execute(query)
...
if __name__ == '__main__':
    try:
        ...
        with getopenconnection() as con:
            ...
            print("My program took", time.time() - start_time, "to load ratings")
            # ###################################################################################
            ...
            print("My program took", time.time() - start_time, "to run rangepartition")


            start_time = time.time()
            rangeinsert('ratings', 2, 299, 0, con)
            print("My program took", time.time() - start_time, "to run rangeinsert")

            start_time = time.time()
            roundrobinpartition('ratings', 3, con)
            print("My program took", time.time() - start_time, "to run roundrobinpartition")


            start_time = time.time()
            roundrobininsert('ratings', 5, 599, 3.5, con)
            print("My program took", time.time() - start_time, "to run roundrobininsert")
