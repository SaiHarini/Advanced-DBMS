...
def rangepartition(ratingstablename, numberofpartitions, openconnection):
	...
	if lowerbound == ratinglowerbound:
		...
		query = " INSERT INTO {0} SELECT * FROM {1} WHERE {2} > {3} and {2} <= {4}".format( partitiontablename,
                                                                                            ...
                                                                                            upperbound )
	cur.execute(query)
	print("QUERY EXECUTED IN rangepartition!")
	openconnection.commit()
...
def roundrobinpartition(ratingstablename, numberofpartitions, openconnection):
	...
	try:
		query = "INSERT INTO {0} " \
    		...
    		cur.execute(query)
    		print("QUERY EXECUTED IN roundrobinpartition!")
		openconnection.commit()
	except Exception as ex:
		print(ex)
    pass
...
if __name__ == '__main__':
    try:
        ...
        with getopenconnection() as con:
            ...
            # loadratings('rat','test_data.dat', con)
            # ###################################################################################
            ...
            # roundrobinpartition
            roundrobinpartition('rat',3,con)

            # rangepartition
            # rangepartition('rat',3,con)

            # roundrobininsert
            # --------roundrobininsert('rat',2,99,0,con)



            # rangeinsert
            # rangeinsert('rat',2,399,0,con)
